...
enc = 'latin1'  # ascii could be enough but latin1 goes up  to 255
...
class CIF(dict):
    ...
    CHEMICAL_NAME_COMMON = b("_chemical_name_common")
    LOOP=b("_loop")
    ...
    def loadCIF(self, _strFilename, _bKeepComment=False):
        ...
        if type(_strFilename) in StringTypes:
    ...
    #        print(lFields[i], lFields[i+1]
            if len(lFields[i + 1]) == 0 :
                lFields[i + 1] = b("?")
            if (lFields[i][0:1] == b("_")) and (lFields[i + 1][0:1] != b("_")):
...
#                        print(lFields[-1]
                    ...
                    elif sText[idx + 1:idx + 2] in CIF.BLANK:
                        lFields.append(sText[1:idx].strip())
#                        print(lFields[-1]
                        sText1 = sText[idx + 1:]
                        sText = sText1.strip()
...
#        curloop = {}
        ...
        while True:
            if i >= len(lFields):
                ...
                break
            elif lFields[i][0:1] == b("_"):
                break
            elif lFields[i] in [CIF.LOOP, b("stop_"), b("global_"), b("data_"), b("save_")]:
                break
            else:
...
#############################################################################################
    ...
    def tostring(self, _strFilename=None, linesep=os.linesep):
        ...
        """
#        sCifText = ""
        lstStrCif = [b("# ") + b(i) for i in __version__]
        if CIF.CHEMICAL_NAME_COMMON in self:
            t = self[CIF.CHEMICAL_NAME_COMMON].split()[0]
        elif _strFilename is not None:
            t = os.path.splitext(os.path.split(str(_strFilename).strip())[1])[0]
        else:
            t = b("")
        lstStrCif.append(b("data_%s" % (t)))
        ...
        for sKey in self._ordered:
            ...
            if sKey not in self:
                ...
                continue
            sValue = bytes(self[sKey])
            if sValue.find(CIF.LF) > -1:  # should add value  between ;;
                lLine = [sKey, ";", sValue, ";", ""]
            elif len(sValue.split()) > 1: #should add value between ''
                sLine = b("%s        '%s'" % (sKey, sValue))
                if len(sLine) > 80:
                    lLine = [bytes(sKey), sValue]
                else:
                    lLine = [sLine]
            else:
                sLine = b("%s        %s" % (sKey, sValue))
            ...
            lstStrCif += lLine
        if CIF.LOOP in self:
            for loop in self[CIF.LOOP]:
                lstStrCif.append(CIF.LOOP+CIF.SPACE)#CHECK for a space
                lKeys = loop[0]
                llData = loop[1]
                lstStrCif += [b(sKey)+CIF.SPACE for sKey in lKeys]
                for lData in llData:
                    sLine = " "
                    for key in lKeys:
                        sRawValue = lData[key]
                        if sRawValue.find(CIF.LF) > -1:  # should add value  between ;;
                            lstStrCif += [sLine, b(";"), bytes(sRawValue), b(";")]
                            sLine = " "
                        else:
                            if len(sRawValue.split()) > 1: #should add value between ''
                                value = b("'%s'" % (sRawValue))
                            else:
                                value = bytes(sRawValue)
    ...
    def existsInLoop(self, sKey):
        ...
        @rtype: boolean
        """
        if not self.exists(CIF.LOOP):
            return False
        bExists = False
        if not bExists:
            for i in self[CIF.LOOP]:
                for j in i[0]:
                    if j == sKey:
                        bExists = True
        return bExists
    ...
    def loadCHIPLOT(self, _strFilename):
        ...
        self[b("_audit_creation_method")] = 'From 2-D detector using FIT2D and CIFfile'
        self[b("_pd_meas_scan_method")] = b("fixed")
        self[b("_pd_spec_description")] = lInFile[0].strip()
        try:
...
#        print("limitsOK:", limitsOK
        for sLine in lInFile[4:]:
            sCleaned = sLine.split(b("#"))[0].strip()
            data = sCleaned.split()
            if len(data) == 2 :
                if not limitsOK:
                    ...
                    if f2Theta > f2ThetaMax :
                        f2ThetaMax = f2Theta
                lOneLoop.append({ b("_pd_meas_intensity_total"): data[1] })
        if not iLenData:
            iLenData = len(lOneLoop)
        assert (iLenData == len(lOneLoop))
        self[ b("_pd_meas_2theta_range_inc") ] = b("%.4f" % ((f2ThetaMax - f2ThetaMin) / (iLenData - 1)))
        if self[ b("_pd_meas_2theta_range_inc")] < 0:
            self[ b("_pd_meas_2theta_range_inc") ] = abs(self[ b("_pd_meas_2theta_range_inc" )])
            tmp = f2ThetaMax
            f2ThetaMax = f2ThetaMin
            f2ThetaMin = tmp
        self[ b("_pd_meas_2theta_range_max") ] = b("%.4f" % f2ThetaMax)
        self[ b("_pd_meas_2theta_range_min") ] = b("%.4f" % f2ThetaMin)
        self[ b("_pd_meas_number_of_points") ] = b(iLenData)
        self[CIF.LOOP] = [ [ [b("_pd_meas_intensity_total") ], lOneLoop ] ]
